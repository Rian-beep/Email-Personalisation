...
Write a short cold email (3-4 sentences max). It must:
1. Open with one specific signal from the company context (a project, a recent win, a public commitment)
2. Connect to one specific operational pain point relevant to construction (earthworks timeline, carbon compliance, diesel cost, urban site constraints, project carbon reporting)
3. End with a concrete next step (a 20-minute call before a specific milestone)

No generic openers. No "I hope this finds you well." Make it sound like a senior operator wrote it.
...
def main():
    ...
    for c in completed_companies:
        with st.expander(f"**{c['company_name']}** — {c.get('director_name', '?')}"):
            col_left, col_right = st.columns([1, 1])
            with col_left:
                st.markdown(f"**Director:** {c.get('director_name')}  ")
                st.markdown(f"**Role:** {c.get('director_role')}  ")
                st.markdown(f"**Email:** {c.get('email') or 'Not found'}  ")
                st.markdown(f"**Email Status:** {c.get('email_status', '—')}  ")
            ...
            with col_right:
                ...
                st.markdown(f"**Body:**")
                st.markdown(f"> {c.get('body', '').replace(chr(10), chr(10) + '> ')}")

            if c.get("company_context"):
                st.markdown("**Company context (Perplexity):**")
