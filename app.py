...
def find_project_manager(company_name):
    """Two-step approach to find the person responsible for a major project at this company.

    Step 1: Ask Perplexity openly about the company's current major projects and named leadership.
    Step 2: Use Anthropic to extract structured data from Perplexity's response.

    Returns dict with: name, role, project_name, project_summary, source_url
    """
    # Step 1: Open-ended Perplexity research (no JSON constraint)
    research_prompt = f"""I need to identify the senior person responsible for a specific major construction project at {company_name} (a UK construction company).

Search publicly available sources including:
- ICE (Institution of Civil Engineers) talks and presentations
- New Civil Engineer (NCE) magazine articles and interviews
- Construction News, Building Magazine, Construction Manager
- The company's own press releases and project announcements
- LinkedIn posts from named team members
- Trade press project announcements

Tell me, in detail:
1. What is one major active or recently completed project at {company_name}?
2. Who is the named senior person leading or responsible for that project? Any role works: Project Director, Operations Director, Construction Director, Senior Project Manager, Major Projects Lead, Civil Engineering Director, Marine Works Director, Programme Director, etc.
3. What does the project involve? Scale, scope, value, timeline.
4. What public sources mention this person and project together?

Be specific. Use real names and real project details from your sources. If you can find multiple candidates, focus on the one with the strongest public profile (interviews, talks, articles).

Don't return JSON. Write naturally in 2-3 paragraphs covering the project, the person responsible, and the source."""
    ...
    # Step 2: Use Anthropic to extract structured data from Perplexity's research
    extract_prompt = f"""Read the research below about {company_name} and extract the named person responsible for a major project, plus the project details.
...
{{
  ...
  "role": "Their actual job title from the research",
  "project_name": "The specific project name",
  "project_summary": "1-2 sentences on the project — scale, scope, value, timeline",
  "source_url": "URL of the source if mentioned, otherwise empty string"
}}

If the research clearly identifies a named person responsible for a specific project, extract them.

If the research mentions multiple people, pick the one most directly responsible for the project (Project Director > Operations Director > general management).

If the research genuinely does not name any person responsible (e.g. only mentions "the team" or "company staff"), return:
{{"name": null, "role": null, "project_name": null, "project_summary": null, "source_url": null}}

Do NOT invent names. Only extract real names that appear in the research text above."""
...
# ============================================================
# Stage 3: Find email via Hunter.io
# ============================================================

def find_email(first_name, last_name, company_name, company_website=""):
    """Use Hunter.io email-finder endpoint to find a verified email.

    Hunter requires at least one of: domain / company / linkedin_handle
    Plus name: first_name + last_name OR full_name OR linkedin_handle
    """
    params = {
    ...
    # Prefer domain if available — Hunter explicitly says domain gives better results
    if company_website:
        # Strip protocol if present
        domain = company_website.replace("https://", "").replace("http://", "").replace("www.", "").split("/")[0].strip()
    ...
    # Hunter returns errors in body['errors'] as a list
    if "errors" in body:
        err = body["errors"][0] if body["errors"] else {}
        return {
            "email": "",
            "status": err.get("code", "ERROR"),
            "raw": body
        }
    ...
    # Hunter status values: valid, accept_all, unknown
    status_raw = verification.get("status", "unknown")

    # Normalise to be readable in the UI
    if status_raw == "valid":
        status_display = "VERIFIED"
    elif status_raw == "accept_all":
        status_display = f"ACCEPT_ALL ({score}%)" if score else "ACCEPT_ALL"
    elif status_raw == "unknown":
        status_display = f"UNKNOWN ({score}%)" if score else "UNKNOWN"
    else:
        status_display = status_raw.upper() if status_raw else "NO_MATCH"

    if not email:
        status_display = "NO_MATCH"
...
def main():
    ...
    with export_col2:
        # CSV of emails that Hunter actually returned (excludes NO_MATCH, ERROR, no email)
        verified = [
            c for c in companies
            if c.get("email")
            and c.get("email_status") not in ("NO_MATCH", "ERROR", "", None)
        ]
        ...
        st.download_button(
            label=f"📥 With email ({len(verified)})",
            data=csv_verified_buffer.getvalue(),
            file_name=f"constructreach_with_email_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
            mime="text/csv",
            use_container_width=True,
